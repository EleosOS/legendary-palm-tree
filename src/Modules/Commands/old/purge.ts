...
const purgeCommand: PalmCommandOptions = {
    ...
    onCancelRun: (ctx) => ctx.editOrReply(Strings.commands.general.usageNotAllowed),
    run: async (ctx, args) => {
        ...
        await ctx.channel
            ...
            .then(async () => {
                const msg = await ctx.editOrReply(Strings.commands.purge.deletedMessages.replace("?", args.amount.toString()));

                setTimeout(await msg.delete({ reason: "Purged messages" }), 15000);
            });
    },
